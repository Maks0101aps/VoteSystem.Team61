  ...
  <div class="relative min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 py-8">
    ...
    <div class="container mx-auto px-4 relative z-10 max-w-4xl">
      ...
      <div class="text-center mb-10">
        <h1 class="text-4xl font-bold text-green-800 mb-2">{{ $t('petitions_page.title') }}</h1>
        <div class="h-1 w-24 bg-green-500 mx-auto mb-4 rounded-full"></div>
        <p class="text-xl text-green-700">{{ $t('petitions_page.active_petitions') }}</p>
      </div>
      ...
      <div class="space-y-6">
        <div v-if="petitions.length === 0" class="bg-white p-8 rounded-xl shadow-md backdrop-blur-sm bg-opacity-90 text-center">
          ...
          <h3 class="text-xl font-medium text-green-700 mb-2">{{ $t('petitions_page.no_active_petitions') }}</h3>
          <p v-if="$page.props.auth.user.role === 'student'" class="text-green-600 mb-6">{{ $t('petitions_page.create_first_petition') }}</p>
        ...
        <div v-for="petition in processedPetitions" :key="petition.id" 
          ...
          <div class="flex justify-between items-start mb-4">
            ...
            <span v-else class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-medium">{{ $t('petitions_page.supported') }}</span>
          </div>
          ...
          <div class="relative mb-4">
            <div class="overflow-hidden h-5 text-xs flex rounded-full bg-gray-200">
              <div :style="{ width: petition.percentage + '%' }" class="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-green-500 transition-all duration-500">
              </div>
            </div>
