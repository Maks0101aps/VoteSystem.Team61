<template>
  <div class="flex items-center justify-between w-full">
    ...
    <div class="absolute left-1/2 transform -translate-x-1/2 flex items-center space-x-1">
      ...
      <Link class="flex items-center justify-center w-10 h-10 rounded-full hover:bg-gray-200/80 transition-colors duration-200" :title="$t('main_menu.voting')" href="/votings">
        <icon name="vote" class="w-5 h-5" :class="isUrl('votings') ? 'fill-green-600' : 'fill-gray-500 group-hover:fill-green-500'" />
      </Link>
      ...
      <Link class="flex items-center justify-center w-10 h-10 rounded-full hover:bg-gray-200/80 transition-colors duration-200" :title="$t('main_menu.reports')" href="/reports">
        ...
        <span v-if="$page.props.pending_petitions_count > 0" class="absolute top-1 right-0 block h-4 w-4 rounded-full bg-red-500 text-white text-[10px] flex items-center justify-center">
          {{ $page.props.pending_petitions_count }}
        </span>
      </Link>
    </div>
