ыв<template>
  <div class="fixed top-5 right-5 z-50 w-full max-w-xs">
    <transition name="slide-fade">
      <div v-if="$page.props.flash.success && show" class="flex items-center justify-between mb-8 rounded-lg bg-green-500 shadow-lg">
        <div class="flex items-center">
          ...
          <div class="py-4 text-white text-sm font-medium">{{ $t($page.props.flash.success) }}</div>
        </div>
    ...
    <transition name="slide-fade">
      <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="flex items-center justify-between mb-8 rounded-lg bg-red-500 shadow-lg">
        <div class="flex items-center">
          ...
          <div v-if="$page.props.flash.error" class="py-4 text-white text-sm font-medium">{{ $t($page.props.flash.error) }}</div>
          <div v-else class="py-4 text-white text-sm font-medium">
            <span v-if="Object.keys($page.props.errors).length === 1">{{ $t('messages.one_form_error') }}</span>
            <span v-else>{{ $t('messages.form_errors', { count: Object.keys($page.props.errors).length }) }}</span>
          </div>
        </div>
        <button type="button" class="group mr-2 p-2" @click="show = false">
